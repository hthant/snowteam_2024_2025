...
# Replace mirrored with central storage here
def getModel(d_num=1):
    devices = tf.config.list_physical_devices('GPU')

    devices_names = [d.name.split("e:")[1] for d in devices]
    if (d_num == 3):
        central_strategy = tf.distribute.exeprimental.CentralStorageStrategy(compute_devices=devices_names)
    else:
        central_strategy = tf.distribute.experimental.CentralStorageStrategy(compute_devices=devices_names[:d_num])
...
# Check if training proccess produces any errors, sending a Discord message to signify success or failure
try:
    for i in range(1, 4):
        ...
        print("Training complete! Total minutes taken to train {} epochs on {} GPUs with {} images: {:.2f} minutes".format(EPOCHS, i, NUMPERCLASS, minutes[0]))
